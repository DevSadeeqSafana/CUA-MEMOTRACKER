...
export default async function UserManagementPage({
    ...
    const users = await query(`
        SELECT u.id, u.uuid, u.username, u.email, u.department, u.is_active, u.staff_id,
               m_mgr.username as manager_name,
               GROUP_CONCAT(r.name) as roles_list
        FROM memo_system_users u
        LEFT JOIN hr_staff hr ON u.staff_id = hr.StaffID
        LEFT JOIN memo_system_users m_mgr ON hr.LineManagerID = m_mgr.staff_id
        LEFT JOIN user_roles ur ON u.id = ur.user_id
        LEFT JOIN roles r ON ur.role_id = r.id
        WHERE (? IS NULL OR u.username LIKE ? OR u.email LIKE ? OR u.staff_id LIKE ?)
        GROUP BY u.id, u.uuid, u.username, u.email, u.department, u.is_active, u.staff_id, m_mgr.username
