...
export default async function EditMemoPage({
    ...
    const formattedBudgetItems = budgetItems.map((item: any) => {
        let budget_category = memo.budget_category || '';
        let cleanDescription = item.description || '';
        const match = (item.description || '').match(/^\[(.*?)\]\s*(.*)$/);
        if (match) {
            budget_category = match[1];
        ...
        <div className="container mx-auto pb-10">
            <EditMemoClient
                ...
                initialData={{
                    ...
                    budget_items: formattedBudgetItems.length > 0 ? formattedBudgetItems : undefined,
                }}
            />
        </div>
    );
